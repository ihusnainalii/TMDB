...
extension DatabaseConfigurable {
    ...
    ///   - completion: completion success or failed
    func delete(entities: [Object], completion: @escaping (Result<Bool, RealmErrorType>) -> Void) {
        guard let realm = self.realm() else {
            completion(.failure(.realmIsEmpty))
            return
        }
    ...
    func deleteAll<T>(deleteObjClass: T.Type, completion: @escaping (Result<Bool, RealmErrorType>) -> Void) where T: Object {
        guard let realm = self.realm() else {
            completion(.failure(.realmIsEmpty))
            return
        }
    ...
    public func query<T>(with predicate: NSPredicate) -> Results<T>? where T: Object {
        guard let realm = self.realm() else {
            return nil
        }
